...
class ReleaseTests(unittest.TestCase):
 ...
 def test_promotion_and_rollback_are_approved_and_atomic(self):
  ...
  with tempfile.TemporaryDirectory(dir=scratch) as d:
   ...
   first=plan_release(DELTA,parent_release_id=None,active_embedding=None,requested_embedding={"model":"a"},selection_fingerprint="s1",removal_mode="reconcile_approved",export_bundle_fingerprint=export_fingerprint(export),vector_representation_id="repr-1"); store.stage_export(first,export); store.promote(first,approved_plan_id=first["plan_id"])
   second=plan_release(DELTA,parent_release_id=first["release_id"],active_embedding={"model":"a"},requested_embedding={"model":"a"},selection_fingerprint="s2",removal_mode="reconcile_approved",export_bundle_fingerprint=export_fingerprint(export),vector_representation_id="repr-1"); store.stage_export(second,export); store.promote(second,approved_plan_id=second["plan_id"])
 ...
 def test_real_export_staging_writes_release_metadata_and_promotes_consumer_path(self):
  ...
  with tempfile.TemporaryDirectory(dir=scratch) as d:
   root=Path(d); store=ReleaseStore(root/"store"); export=self.write_export(root)
   plan=plan_release(DELTA,parent_release_id=None,active_embedding=None,requested_embedding={"model":"a"},selection_fingerprint="s",removal_mode="reconcile_approved",export_bundle_fingerprint=export_fingerprint(export),vector_representation_id="repr-1")
 ...
 def test_retention_never_deletes_without_separate_approval(self):
  ...
  with tempfile.TemporaryDirectory(dir=scratch) as d:
   ...
   for index in range(3):
    plan=plan_release(DELTA,parent_release_id=store.active(),active_embedding=None,requested_embedding={"model":"a"},selection_fingerprint=str(index),removal_mode="reconcile_approved",export_bundle_fingerprint=export_fingerprint(export),vector_representation_id="repr-1"); store.stage_export(plan,export); store.promote(plan,approved_plan_id=plan["plan_id"]); releases.append(plan["release_id"])
 ...
 def test_release_cli_runs_validated_delta_to_consumer_readable_release(self):
  ...
  with tempfile.TemporaryDirectory(dir=scratch) as d:
   ...
   embedding=root/"embedding.json"; embedding.write_text(json.dumps({"model":"a"}),encoding="utf-8"); plan_path=root/"plan.json"; store_path=root/"store"
