param(
    ...
    [switch]$Rebuild,
    [switch]$Update,
    [switch]$EmbeddingBenchmark,
    [switch]$DiagnosticBundle,
    [switch]$ReleaseBuild,
    [switch]$ListCollections,
    [switch]$InspectCollection,
...
if (-not (Test-CondaEnv)) {
    ...
    Write-Host "  .\scripts\Run-ChromaDbImport.ps1 -CreateCondaEnv"
    exit 1
}
...
if ($EmbeddingBenchmark) {
    $argsList += "--embedding-benchmark"
}
if ($DiagnosticBundle) {
    $argsList += "--diagnostic-bundle"
}
if ($ReleaseBuild) {
    $argsList += "--release-build"
}
if ($ListCollections) {
    $argsList += "--list-collections"
}
if ($InspectCollection) {
    $argsList += "--inspect-collection"
}
if ($DeleteCollection) {
    $argsList += "--delete-collection"
}

$pythonArgs = @("-m", "chroma_db_import") + $argsList
