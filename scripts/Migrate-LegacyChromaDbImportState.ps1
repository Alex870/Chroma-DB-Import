...
function Select-LegacyFolder {
    ...
    $dialog = New-Object System.Windows.Forms.FolderBrowserDialog -Property @{
        ...
        Description = "Select the legacy Chroma DB Import directory to migrate from."
    }
...
$legacyConfigPath = Join-Path $resolvedLegacyRoot "chroma_db_import_config.json"
$legacyConfig = $null
...
$configSpecs = @(
    [pscustomobject]@{ Key = "processed_data_dir"; Label = "Processed cache directory"; Type = "Directory"; Default = "processed_data"; CopyMode = "progress"; Optional = $true },
    [pscustomobject]@{ Key = "state_path"; Label = "Import state file"; Type = "File"; Default = "state/chroma_import_state.json"; CopyMode = "file"; Optional = $true },
    [pscustomobject]@{ Key = "stop_file"; Label = "Stop-after-current import request"; Type = "File"; Default = "state/stop_after_current_import.txt"; CopyMode = "skip"; Optional = $true },
    [pscustomobject]@{ Key = "persist_dir"; Label = "Chroma persist directory"; Type = "Directory"; Default = "chroma_db_raptor_v2"; CopyMode = "merge"; Optional = $true },
    [pscustomobject]@{ Key = "manifest_path"; Label = "Import manifest file"; Type = "File"; Default = "import_manifest.json"; CopyMode = "file"; Optional = $true; RelativeTo = "persist_dir" },
    [pscustomobject]@{ Key = "preflight_report_path"; Label = "Preflight report"; Type = "File"; Default = "state/preflight_report.json"; CopyMode = "file"; Optional = $true },
    [pscustomobject]@{ Key = "import_state_dir"; Label = "Import batch state directory"; Type = "Directory"; Default = "state/import_batches"; CopyMode = "merge"; Optional = $true },
    [pscustomobject]@{ Key = "embedding_cache_dir"; Label = "Embedding cache directory"; Type = "Directory"; Default = "state/embedding_cache"; CopyMode = "merge"; Optional = $true },
    [pscustomobject]@{ Key = "troubleshooting_dir"; Label = "Diagnostics directory"; Type = "Directory"; Default = "state/diagnostics"; CopyMode = "merge"; Optional = $true }
)
...
$plannedTargets.Add([pscustomobject]@{ TargetPath = (Join-Path $resolvedProjectRoot "chroma_db_import_config.json") }) | Out-Null

$resolvedValues = @{}
foreach ($spec in $configSpecs) {
    if ($spec.PSObject.Properties["RelativeTo"] -and $resolvedValues.ContainsKey($spec.RelativeTo)) {
        $baseResolved = $resolvedValues[$spec.RelativeTo]
        ...
        } elseif ($baseResolved -and $baseResolved.RelativePath) {
            $relativeCombined = Join-Path $baseResolved.RelativePath $configuredValue
            $legacyCandidate = Join-Path $resolvedLegacyRoot $relativeCombined
            $resolved = [pscustomobject]@{
                ConfigValue  = $configuredValue
                RelativePath = $relativeCombined
                PortablePath = $configuredValue.Replace('\', '/')
                LegacyPath   = if (Test-PathTypeMatch -Path $legacyCandidate -PathType $spec.Type) { $legacyCandidate } else { $null }
                Resolution   = if (Test-PathTypeMatch -Path $legacyCandidate -PathType $spec.Type) { "relative_to_persist_dir" } else { "relative_missing" }
                ShouldRemap  = $false
            }
        } else {
            $resolved = Resolve-ConfiguredPath -ConfiguredPath $configuredValue -LegacyRoot $resolvedLegacyRoot -CurrentProjectRoot $resolvedProjectRoot -PathType $spec.Type
    ...
    $resolvedValues[$spec.Key] = $resolved
...
if (Test-Path -LiteralPath $legacyConfigPath -PathType Leaf) {
    Copy-FileWithBackup -SourcePath $legacyConfigPath -TargetPath (Join-Path $resolvedProjectRoot "chroma_db_import_config.json") -Label "Runtime config"
} else {
...
Update-MigratedConfigPaths -ConfigPath (Join-Path $resolvedProjectRoot "chroma_db_import_config.json") -PathMappings $configPathMappings
