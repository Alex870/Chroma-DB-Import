...
def main(argv=None):
    ...
    job_plan=c.add_parser("plan-job"); job_plan.add_argument("plan"); job_plan.add_argument("--export",required=True); job_plan.add_argument("--store",required=True); job_plan.add_argument("--jobs",required=True)
    ...
    if a.command=="plan-job":
        value=plan_job(_read(a.plan),a.export,a.store,ReleaseJobStore(a.jobs)); print(value["job_id"]); return 0
    ...
    value=_read(a.plan)
    if a.command=="stage":
        if a.export: store.stage_export(value,a.export)
